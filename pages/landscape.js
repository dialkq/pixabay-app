...
import { getPhotoLandscape } from '../library/api';
import InfiniteScroll from 'react-infinite-scroll-component';

// const fetchPhoto = async () => {
// 	const res = await fetch('https://pixabay.com/api/?key=19323544-16a5578b0be625367b264af08&image_type=photo&per_page=100&orientation=horizontal&category=buildings');
// 	return res.json();
// }
...
const Landscape = () => {
  ...
  //Fetcher First Time
  const fetchPhoto = async () => {
    const res = await axios.get(`${getPhotoLandscape(page)}`);
  ...
  //Masonry Layout
	...
	//React Query
	const {data,status,error} = useQuery('landscape',fetchPhoto,
  {
    staleTime: 1000,
    cacheTime: 10,
          ...
          <div className="w-11/12 mx-auto my-3 md:my-4 lg:my-6 xl:my-6">
            <Navbar />
					...
					<div className="w-11/12 mx-auto my-10 md:my-14 lg:my-20">
						<p className="text-center font-mono text-lg lg:text-xl xl:text-xl font-bold underline">Landscape</p>
					</div>
